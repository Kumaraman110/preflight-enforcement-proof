# Preflight v0.10.0-rc.2 — Release notes

> **Supersedes v0.10.0-rc.1 for evaluation.** The `v0.10.0-rc.1` tag is immutable and stays in place.
> This rc.2 fixes two issues found during rc.1 burn-in (see below); the push engine is byte-identical
> to rc.1 — only the shell-structure lexer and the user-install uninstall path changed.
...
- **Alternate git-context push bypass (stable blocker):** `git -C <dir> push` / `--git-dir[=]` /
  ...
  `lib/shell-structure-lexer.awk` (skip git global options before the subcommand). These forms now get
  the normal policy decision, or a fail-closed BLOCK when the target repo's config is unreachable —
  never a silent allow. Non-push commands are unaffected.
- **Uninstall restores prior `settings.json` byte-for-byte** (previously preserved keys but reformatted).


## This is a release candidate

`v0.10.0-rc.2` is a **release candidate**, published as a GitHub **prerelease** — it is **not** GA and
...
- **Source commit:** the `release/v0.10.0-rc.2` head. Its product content is the completed **PR #13**
...
tools/preflight-user.sh install --user --ref v0.10.0-rc.2
# or from the downloaded self-contained artifact:
tools/preflight-user.sh install --user --ref v0.10.0-rc.2 --from-artifact preflight-user-v0.10.0-rc.2.tar.gz
...
```bash
sha256sum -c preflight-user-v0.10.0-rc.2.tar.gz.sha256
```
