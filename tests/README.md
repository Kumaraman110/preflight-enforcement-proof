...
Every change to the rubric, promotion criteria, external-review-handler classification, or coupling logic is a hypothesis. Without tests, the first feedback comes from running on a real service (expensive, slow, no rollback). These tests give 30-second validation before any change takes effect.
...
| `operative` | Promotion criteria wiring (Survived counts, Confidence thresholds for rubric-edit PR) | After changing external-review-handler promotion criteria or rubric-edit cadence |
| `coupling` | Structural coupling signals detected correctly | After changing dependency-map-validator |
| `crosscheck` | CONTRADICTS_RUBRIC classification exists and is ordered correctly | After changing external-review-handler stability filter |
...
These tests verify the STRUCTURE of the review system (does the rubric cover X? does the external-review-handler have step Y?). They do NOT invoke the LLM — that would make them non-deterministic. LLM behavior is tested via the fixtures: if the rubric has the detection pattern, the reviewer WILL find it (because the reviewer walks the rubric exhaustively by design).
