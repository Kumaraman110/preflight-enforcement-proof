...
# Build the hook stdin JSON by feeding the command to jq over STDIN (jq -Rs), NOT as an --arg. On
# bash-5.3-cygwin jq is a native Windows exe, so MSYS applies POSIX->Windows argv PATH-CONVERSION to an
# --arg value that looks like `NAME=/abs/path` (e.g. B3 `GIT_DIR=/no/such/dir/.git git push …` was rewritten
# to `GIT_DIR=C:/Program Files/Git/no/such/dir/.git …`, INJECTING A SPACE → the engine then saw an unknown
# leading program before `git push` → wrapper-taxonomy CONFIRM (RC=0) instead of BLOCK). Passing the command
# on stdin (`printf '%s' "$cmd" | jq -Rs …`) bypasses argv conversion entirely and round-trips ANY path shape
# (space paths, D:\a\repo backslash paths, nested bash -c) byte-identically. A test-harness fix; product is OK.
run_hook() { local cmd="$1" json; json="$(printf '%s' "$cmd" | jq -Rs '{tool_name:"Bash",tool_input:{command:.}}')"; OUT="$(printf '%s' "$json" | _PFG_WATCHDOG_CHILD=1 bash "$HOOK" 2>&1)"; RC=$?; }
